...
getJid: {
    async value(sender) {
        ...
        for (const chat of Object.values(conn.chats || {})) {
            ...
            const participant = participants.find(p => p?.lid === jid || p?.id === jid || p?.jid === jid)
            if (!participant) continue
            const resolved = participant.phoneNumber || participant.jid
...
getNum: {
    async value(jid, chatId = null) {
        if (!jid) return null
        let realJid = this.decodeJid(jid)
        if (realJid.endsWith('@lid')) {
            realJid = await this.getJid(realJid)
            if (realJid.endsWith('@lid') && chatId) {
                try {
                    const metadata = await this.groupMetadata(chatId)
                    const participant = metadata?.participants?.find(p => p?.lid === realJid || p?.id === realJid)
                    if (participant?.phoneNumber) {
                        this.isLid.set(realJid, participant.phoneNumber)
                        realJid = participant.phoneNumber
                    }
                } catch {}
            }
        }
        return realJid ? realJid.split('@')[0].split(':')[0] : null
    },
    enumerable: true
},
getName: {
    async value(jid = '', withoutContact = false) {
        try {
            ...
            if (jid.endsWith('@g.us')) {
                const group = this.chats?.[jid]
                if (group?.subject) return group.subject
                const meta = await this.groupMetadata(jid).catch(() => null)
                return meta?.subject || 'Grupo'
            }
            if (jid === this.user?.jid || jid === this.user?.id) return this.user?.name || 'Bot'
            let chat = this.chats?.[jid] || {}
            let contact = this.contacts?.[jid] || {}
            if (jid.endsWith('@lid')) {
                const realJid = await this.getJid(jid)
                if (realJid !== jid) {
                    chat = { ...(this.chats?.[realJid] || {}), ...chat }
                    contact = { ...(this.contacts?.[realJid] || {}), ...contact }
                }
                for (const group of Object.values(conn.chats || {})) {
                    const participant = group?.metadata?.participants?.find(p => p?.lid === jid || p?.id === jid)
                    if (participant) {
                        return (!withoutContact && participant.name) || participant.notify || participant.pushName || contact.name || contact.notify || chat.name || chat.notify || participant.phoneNumber?.split('@')[0] || jid.split('@')[0]
                    }
                }
            }
            return (!withoutContact && contact.name) || contact.notify || chat.name || chat.notify || jid.split('@')[0]
        } catch {
            return jid.split('@')[0]
        }
    }
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        sender: {
            get() {
                let jid = this.key?.fromMe ? this.conn?.user?.id : this.participant || this.key?.participant || this.chat || ''
                if (!jid) return jid
                return this.conn?.decodeJid(jid) || jid
            },
            enumerable: true
        },
        fromMe: {
