...
function sticker6(img, url) {
  return new Promise(async (resolve, reject) => {
    ...
    const safeExtMap = { jpg: 'jpg', jpeg: 'jpeg', png: 'png', gif: 'gif', webp: 'webp', mp4: 'mp4', mov: 'mov', avi: 'avi', mkv: 'mkv', '3gp': '3gp', webm: 'webm' }
    if (type.ext == 'bin' || !Object.prototype.hasOwnProperty.call(safeExtMap, type.ext)) reject(img)
    const safeExt = safeExtMap[type.ext]
    // Extension is not interpolated into the path; only a fixed literal is used for the temp file name.
    const tmp = path.join(__dirname, `../tmp/${+ new Date()}.input`)
    const out = path.join(tmp + '.webp')
    await fs.promises.writeFile(tmp, img)
    // https://github.com/MhankBarBar/termux-wabot/blob/main/index.js#L313#L368
    let Fffmpeg = /video/i.test(type.mime) ? fluent_ffmpeg(tmp).inputFormat(safeExt) : fluent_ffmpeg(tmp).input(tmp)
    Fffmpeg
